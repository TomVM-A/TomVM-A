<section id="presentation">
    <p align = "center">
        <img src = "https://komarev.com/ghpvc/?username=TomVM-A&style=plastic&color=blueviolet" alt = "Profile Views"/>
    </p>
    <h2 style="text-align: center;">Hello there 👋</h2>
    <p aling="center">
        <img src="https://media.giphy.com/media/v1.Y2lkPWVjZjA1ZTQ3MThtazV6YWxxaDFmOGdlYm1xcTFoZDBrbzBveWRocDQ5ODV3N2luMCZlcD12MV9naWZzX3JlbGF0ZWQmY3Q9Zw/3o6nV966gHSu47JbUY/giphy.gif" alt="Arnol, from hey arnol! show is say hello">
    </p>
    <h1 style="text-align: center;">I'm Tomas, a <strong>backend developer 🧑‍💻</strong></h1>
</section>

I view software development as the ultimate tool-creation engine 🚀. My passion is building impactful solutions that address real-world challenges—a mindset reinforced by my background in **Environmental Engineering**.

---

### 💻 My Core Stack & Experience

* **Professional Experience:** 1 year leveraging Ruby and Rails to develop robust backend solutions.
* **Focus:** Backend development, problem-solving, and sustainable tool creation.

### 📈 Current Learning Trajectory

I am actively expanding my skillset to cover the full SDLC, with a strong focus on:

* **DevOps:** Mastering Continuous Integration / Continuous Deployment (CI/CD).
* **Front-End:** Building modern interfaces using JavaScript and **React**.
* **Data Science:** Exploring the application of AI/Machine Learning principles via **Python**.

---

### 🌐 Let's Connect!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/tomas-vargas-montes/)
